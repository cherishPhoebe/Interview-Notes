...
## `as` 和 `is` 的区别  
在C#中 `as` 和 `is` 都是用于类型检查和转换的操作符，他们的用途和行为有显著区别  
1. **`is` 操作符**  
  - 用途：检查对象是否与指定类型兼容（包括继承、接口实现）
  - 返回值：布尔值（true/false）
  - 行为：
    - 若对象是目标类型（或其派生类），或实现了目标接口，则返回 true
    - 若对象为 null 或 类型不兼容，返回 false
    - 不会抛出异常
  - 支持模式匹配（C# 7.0+）：可直接声明转换后的变量： eg``` if(obj is string str) ``` 可直接使用str
  - 适用类型：所有类型（引用类型、值类型）  
2. **`as` 操作符**
  - 用途：尝试将对象安全地转换为目标类型
  - 返回值： 转换成功则返回目标类型的引用，否则返回 null
  - 行为：
    - 仅适用于**引用类型**或**可为空的值类型(int?)**
    - 不会抛出异常
  - 限制：不可用于非可空值类型（如：int、struct）  
